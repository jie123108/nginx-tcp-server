...
int ParseArgs(int argc, char* argv[], char* host, 
	...
	while((result = getopt(argc, argv, "h:p:t:c:f:r:l:?")) != -1){
		...
		break;
		//default:
			//LOG_ERROR("Unknow option -%c", result);
			//exit(0);
		}
	}
			...
			FClientCtxNew ClientNewCb, FClientCtxFree ClientFreeCb,int expectedCode, int num)
{
	if(threads > MAX_TEST_THREADS){
		LOG_ERROR("threads(%d) > MAX_TEST_THREADS(%d)", threads, MAX_TEST_THREADS);
		return -1;
	}
	...
	for(i=0;ClientNewCb != NULL && i < connections; i++){
		...
		if(g_ctxs[i] == NULL){
			LOG_ERROR("Connec to [%s:%d] failed!", ip,port);
			return -1;
		}
	}
	LOG_INFO("#################  Connect End  #################");
	...
	for(i=0;i < threads; i++){
		...
		LOG_INFO("########## One Thread One Socket ##############");
