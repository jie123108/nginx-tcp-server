...
BLOCK_RECV:
	while(recv_cnt < iMsgLen){
		...
		if(ret == 0){
			LOG_ERROR("Recv Msg error! socket closed!");
			return ret;
		}else if(ret < 0)
		{
			if(errno == EINTR){
...
int sock_set_nonblock(int socket){
	...
	if(opts <0){
		LOG_ERROR("fcntl(sock, F_GETFL) failed!");
		return -1;
	}
...
int sock_set_cork(int socket, int on){
	...
	if(ret == -1){
		LOG_ERROR("Set Sock CORK Failed! err=%s", strerror(errno));
	}
...
int sock_set_linger(int socket, int second)
{
	...
	if(ret == -1){
		LOG_ERROR("Set Sock Linger Failed! err=%s", strerror(errno));
	}
...
int sock_set_reuseaddr(int socket){
	...
	if(ret == -1){
		LOG_ERROR("Set Sock ReuseAddr Failed! err=%s", strerror(errno));
	}
...
int sock_set_keepalive(int socket, int iInterval, int iCount)
{
	...
	if(ret == -1){
		LOG_ERROR("Set Sock[%d] KeepAlive Failed! err=%s",socket,  strerror(errno));
		return ret;
	}
	
	ret = setsockopt(socket, SOL_TCP , TCP_KEEPIDLE , (const char*)&iInterval , sizeof(iInterval) );
	if(ret == -1){
		LOG_ERROR("Set Sock KeepAlive Idle Failed! err=%s", strerror(errno));
		return ret;
	}
		 
	ret = setsockopt(socket, SOL_TCP , TCP_KEEPINTVL , (const char*)&iInterval,sizeof(iInterval) );
	if(ret == -1){
		LOG_ERROR("Set Sock KeepAlive Interval Failed! err=%s", strerror(errno));
		return ret;
	}
		 
	ret = setsockopt(socket, SOL_TCP , TCP_KEEPCNT , (const char*)&iCount,sizeof(iCount) );
	if(ret == -1){
		LOG_ERROR("Set Sock KeepAlive Count Failed! err=%s", strerror(errno));
		return ret;
	}
...
int client_init(client_ctx_t* ctx, const char* ip, int port, int timeout_send,int timeout_recv)
{
	...
	if(ret == 0){
		...
		if(timeout_recv > 0){
			//LOG_DEBUG("########### recv timeout:%d #############", ctx->timeout_recv);
