...
int thread_set_affinity(int cpu_no){
	if(cpu_no >= get_cpu_count()){
		LOG_ERROR("Invalid cpu no[%d], the max no of cpu is %d", 
	...
	if(ret == -1) 
	{ 
		LOG_ERROR("could not set CPU affinity!"); 
	}
...
int CThread::Start() {
	if (m_thread != 0)
	{
		LOG_ERROR(" %%%%%%%% WARN: Thread is started %%%%%%%%%%");
	    return -1;
	}
	...
	if( pthread_attr_setstacksize( &attr , m_stackSize*1024 )){
		LOG_ERROR( "pthread_attr_setstacksize is failed!!" ) ;
	...
	if(ret != 0)
	{
		LOG_ERROR("CThread.Start is faild");
	}
...
int CThread::Join() {
	if(m_isJoinable){
		if(m_thread != 0) {
			...
			if(ret != 0){
				LOG_ERROR("pthread join [%d] faild!", (int)pthread_self());
			}
			m_thread = 0;
		}
		return 0;
	}else{
		LOG_ERROR("Join faild! thread is unjoinable!");
		return -1;
	}
}
...
unsigned CThread::ThreadProc(void* param) {
	...
	if(!p->m_isJoinable){
		...
		if(ret != 0){
			LOG_ERROR("detach thread[%d] faild!", (int)pthread_self());
		}
	}
