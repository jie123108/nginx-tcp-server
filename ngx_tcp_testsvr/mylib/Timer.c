...
int CTimer::InitThreadTimer(TimerProc proc, void* proc_arg, size_t threadStackSize){
	if(m_timer != 0){
		LOG_ERROR("Timer Inited ! m_timer(=%ld) != 0", (long)m_timer);
		return -1;
	}
...
int CTimer::InitSignalTimer(TimerProc proc, void* proc_arg){
	...
	if(sigaction(SIGUSR2, &action, NULL) <0){
		LOG_ERROR("sigactin failed! err:%s", strerror(errno));
		return -1;
	}
...
int CTimer::RestartTimer(double first, double interval){
	...
	if(m_first != first || m_interval != interval){
		LOG_INFO("Timer [%s] Restarted [first:%.3lf, interval:%.3lf]", m_name, first, interval);
		StopTimer();
...
int CTimer::StartTimer(double first, double interval){
	if(m_timer == 0){
		LOG_ERROR("Timer Not Tnitialized!");
		return -1;
	}

	//first == 0
	if(fabs(first) < 0.000000001){
		LOG_ERROR("arg first [%lf] Is Invalid!", first);
		return -1;
	}
	//interval == 0
	if(fabs(interval) < 0.000000001){
		LOG_ERROR("arg interval [%lf] Is Invalid!", interval);
		return -1;
	}
	...
	/* LOG_DEBUG("Timer(%s) Start! First Run[%.6lf], Interval[%.6lf]",
