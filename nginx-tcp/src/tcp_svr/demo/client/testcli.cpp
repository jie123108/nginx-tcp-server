...
inline int sock_send_req(int sockfd,  req_header_t* req_header, const void* body){
	...
	if(ret == -1){
		NLOG_ERROR("send req_header failed! ret=%d, err:%s", ret, strerror(errno));
		return -1;
	}
	sendlen += ret;
	ret = sock_send_all(sockfd, body, req_header->len);
	if((uint32_t)ret != req_header->len){
		NLOG_ERROR("send req body failed! ret=%d, err:%s", ret, strerror(errno));
		return -1;
	}
	sendlen += ret;
...
inline int sock_recv_rsp(int sockfd, rsp_header_t* req_header, void* rsp_body, int* bodylen){
	...
	if(ret != sizeof(rsp_header_t)){
		NLOG_ERROR("recv rsp_header failed! ret=%d, err:%s", ret, strerror(errno));
		return ERRNO_SOCK_ERR;
	}

	if(rsp_header.code != 0){
		NLOG_ERROR("rsp result: %d", rsp_header.code);
	...
	if(len > 0){
		if(*bodylen < len){
			NLOG_ERROR("rsp body len (%d) < rsp_header.len(%d)", *bodylen, len);
			return -1;
		}
		ret = sock_recv_all(sockfd, rsp_body, len);
		if(ret != len){
			NLOG_ERROR("recv body len [%d] != bodylen", ret, len);
			return ERRNO_SOCK_ERR;
		}
		*bodylen = len;
	}
...
int cli_test_init(client_ctx_t* ctx)
{
	...
	if((uint32_t)ret != reqheader.len+sizeof(req_header_t)){
		NLOG_ERROR("sock_send_req failed!, ret=%d, reqheader.reqlen=%d", ret, reqheader.len);
...
int cli_test_add(client_ctx_t* ctx, int n, int* result)
{
	...
	if((uint32_t)ret != reqheader.len+sizeof(req_header_t)){
		NLOG_ERROR("sock_send_req failed!, ret=%d, reqheader.len=%d", ret, reqheader.len);
		ctx->err = ERRNO_SOCK_ERR;
		ctx->needclose = 1;
		return ctx->err;
	}
