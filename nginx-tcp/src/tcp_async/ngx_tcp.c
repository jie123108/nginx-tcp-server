...
ngx_tcp_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *(ngx_tcp_conf_ctx_t **) conf = ctx;
    ...
    ctx->main_conf = (void**)ngx_pcalloc(cf->pool,sizeof(void *) * ngx_tcp_max_module);
    ...
    cscfp = (ngx_tcp_core_srv_conf_t**)cmcf->servers.elts;
    ...
    *cf = pcf;
    ...
    listen = (ngx_tcp_listen_t*)cmcf->listen.elts;
    ...
    ngx_tcp_listen_t *listen)
{
    ...
    port = (ngx_tcp_conf_port_t*)ports->elts;
    ...
    port = (ngx_tcp_conf_port_t*)ngx_array_push(ports);
    if (port == NULL) {
        return NGX_ERROR;
    }
...
found:
    ...
    addr = (ngx_tcp_conf_addr_t*)ngx_array_push(&port->addrs);
    if (addr == NULL) {
        return NGX_ERROR;
    }
...
ngx_tcp_optimize_servers(ngx_conf_t *cf, ngx_array_t *ports)
{
    ...
    for (p = 0; p < ports->nelts; p++) {
        ...
        addr = (ngx_tcp_conf_addr_t*)port[p].addrs.elts;
        ...
        while (i < last) {
            ...
            mport = (ngx_tcp_port_t*)ngx_palloc(cf->pool, sizeof(ngx_tcp_port_t));
            if (mport == NULL) {
                return (char*)NGX_CONF_ERROR;
            }
    ...
    ngx_tcp_conf_addr_t *addr)
{
    ...
    addrs = (ngx_tcp_in_addr_t*)mport->addrs;
